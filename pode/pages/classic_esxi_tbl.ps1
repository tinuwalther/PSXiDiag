                    ...
                    foreach($item in $VIServer){
                        ...
                        New-PodeWebCard -Name "VC$($i)" -DisplayName "vCenter $($vCenter)" -Content @(
                            ...
                            foreach($Cluster in $VICluster){
                                ...
                                New-PodeWebTable -Id $ii -Name "VC$($ii)" -DisplayName "Cluster $($Cluster)" -SimpleSort -Click -Compact -NoExport -NoRefresh -ArgumentList @($Properties, $item, $PodeDB, $SqlTableName, $Cluster) -ScriptBlock {
